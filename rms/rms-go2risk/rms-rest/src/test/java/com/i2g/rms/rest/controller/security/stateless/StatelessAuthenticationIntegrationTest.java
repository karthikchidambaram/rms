...
public class StatelessAuthenticationIntegrationTest {
		...
		public void testUserApi_Get_User() {
			final String result = doAuthenticatedExchange("beena", HttpMethod.GET, "/api/users/current");
			assertTrue(result.contains("\"username\":\"beena\""));
		...
		public void testUserApi_Get_Admin() {
			final String result = doAuthenticatedExchange("karthik", HttpMethod.GET, "/api/users/current");
			assertTrue(result.contains("\"username\":\"karthik\""));
		...
		public void testAdminApi_Get_Users_AsUser() {
			doAuthenticatedExchange("beena", HttpMethod.GET, "/admin/api/users");
		}
		...
		public void testAdminApi_Get_Users_AsAdmin() {
			...
			assertEquals("[{\"id\":1,\"username\":\"karthik\",\"expires\":0,\"roles\":[\"ADMIN\"]}"
					+ ",{\"id\":2,\"username\":\"beena\",\"expires\":0,\"roles\":[\"USER\"]}]", result);
		}
		...
		private String doAnonymousExchange(final HttpMethod method, final String path, String request) {
			...
			HttpEntity<String> testResponse = restTemplate.exchange("http://localhost:8088/" + path, method, testRequest, String.class);
		...
		private String doAuthenticatedExchange(final String user, final HttpMethod method, final String path,
			...
			ResponseEntity<Void> results = restTemplate.postForEntity("http://localhost:8088/rmsrest/api/login", login, Void.class);

			httpHeaders.add("X-AUTH-TOKEN", results.getHeaders().getFirst("X-AUTH-TOKEN"));
			HttpEntity<String> testRequest = new HttpEntity<>(request, httpHeaders);
			HttpEntity<String> testResponse = restTemplate.exchange("http://localhost:8088/" + path, method, testRequest,
					String.class);
			return testResponse.getBody();
		}
}
