 ...
 */
@Repository
public class AssetCategoryDaoImpl extends AbstractHibernateDao<String, AssetCategory> implements AssetCategoryDao {

	private final Logger _logger = LoggerFactory.getLogger(AssetCategoryDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link AssetCategoryDaoImpl}.
	 */
	private AssetCategoryDaoImpl() {
	...
	public List<AssetCategory> get() {
		return (List<AssetCategory>) applySearch(getSession().createCriteria(_modelType)).list();
	}
	...
	@Override
	public AssetCategory getByCode(final String code) {
		// Validate input parameter(s) if any..
		validateCode(code);
		return (AssetCategory) applySearch(getSession().createCriteria(_modelType)
	 ...
	 */
	@Override
	public AssetCategory create(final String code, final String description) {
		// Validate input parameter(s) if any..
		validateCode(code);
		validateDescription(description);
		// Create the new object (record)
		AssetCategory object = new AssetCategory(code, description);
		// Issue save
		save(object);
		return object;
	}
	 ...
	 */
	@Override
	public AssetCategory update(final String code, final String description) {
		// Validate input parameter(s) if any..
		validateCode(code);
		validateDescription(description);
		// Check if the record exists before issuing the update.
		AssetCategory object = getByCode(code);
