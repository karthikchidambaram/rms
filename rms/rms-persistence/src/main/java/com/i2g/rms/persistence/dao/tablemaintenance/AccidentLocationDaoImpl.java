 ...
 */
@Repository
public class AccidentLocationDaoImpl extends AbstractHibernateDao<String, AccidentLocation> implements AccidentLocationDao {

	private final Logger _logger = LoggerFactory.getLogger(AccidentLocationDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link AccidentLocationDaoImpl}.
	 */
	private AccidentLocationDaoImpl() {
		super(AccidentLocation.class);
	}
	...
	public List<AccidentLocation> get() {
		return (List<AccidentLocation>) applySearch(getSession().createCriteria(_modelType)).list();
	}
	...
	public AccidentLocation getByCode(final String code) {
		...
		validateCode(code);
		return (AccidentLocation) applySearch(getSession().createCriteria(_modelType)
	...
	public AccidentLocation create(final String code, final String description) {
		...
		AccidentLocation object = new AccidentLocation(code, description);
		// Issue save
		save(object);
		return object;
	}
	...
	public AccidentLocation update(final String code, final String description) {
		...
		AccidentLocation object = getByCode(code);
