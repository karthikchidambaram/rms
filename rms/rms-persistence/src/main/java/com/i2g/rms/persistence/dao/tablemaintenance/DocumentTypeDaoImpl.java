 ...
 */
@Repository
public class DocumentTypeDaoImpl extends AbstractHibernateDao<String, DocumentType> implements DocumentTypeDao {

	private final Logger _logger = LoggerFactory.getLogger(DocumentTypeDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link DocumentTypeDaoImpl}.
	 */
	private DocumentTypeDaoImpl() {
	...
	public List<DocumentType> get() {
		return (List<DocumentType>) applySearch(getSession().createCriteria(_modelType)).list();
	}
	...
	public DocumentType getByCode(final String code) {
		...
		validateCode(code);
		return (DocumentType) applySearch(getSession().createCriteria(_modelType)
	...
	public DocumentType create(final String code, final String description) {
		...
		DocumentType object = new DocumentType(code, description);
		// Issue save
		save(object);
		return object;
	}
	...
	public DocumentType update(final String code, final String description) {
		...
		DocumentType object = getByCode(code);
