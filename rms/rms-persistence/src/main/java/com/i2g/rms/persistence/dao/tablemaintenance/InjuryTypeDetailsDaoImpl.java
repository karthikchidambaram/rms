 ...
 */
@Repository
public class InjuryTypeDetailsDaoImpl extends AbstractHibernateDao<String, InjuryTypeDetails> implements InjuryTypeDetailsDao {

	private final Logger _logger = LoggerFactory.getLogger(InjuryTypeDetailsDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link InjuryTypeDetailsDaoImpl}.
	 */
	private InjuryTypeDetailsDaoImpl() {
		super(InjuryTypeDetails.class);
	}
	...
	public List<InjuryTypeDetails> get() {
		return (List<InjuryTypeDetails>) applySearch(getSession().createCriteria(_modelType)).list();
	}
	...
	public InjuryTypeDetails getByCode(final String code) {
		...
		validateCode(code);
		return (InjuryTypeDetails) applySearch(getSession().createCriteria(_modelType)
	...
	public InjuryTypeDetails create(final String code, final String description) {
		...
		InjuryTypeDetails object = new InjuryTypeDetails(code, description);
		// Issue save
		save(object);
		return object;
	}
	...
	public InjuryTypeDetails update(final String code, final String description) {
		...
		InjuryTypeDetails object = getByCode(code);
