 ...
 */
@Repository
public class BodyPartDaoImpl extends AbstractHibernateDao<String, BodyPart> implements BodyPartDao {

	private final Logger _logger = LoggerFactory.getLogger(BodyPartDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link BodyPartDaoImpl}.
	 */
	private BodyPartDaoImpl() {
	...
	public BodyPart getByCode(final String code) {
		...
		validateCode(code);
		return (BodyPart) applySearch(getSession().createCriteria(_modelType)
	...
	public BodyPart create(final String code, final String description) {
		...
		BodyPart object = new BodyPart(code, description);
		// Issue save
		save(object);
		return object;
	}
