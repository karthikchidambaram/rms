 ...
 */
@Repository
public class DocumentCategoryDaoImpl extends AbstractHibernateDao<String, DocumentCategory> implements DocumentCategoryDao {

	private final Logger _logger = LoggerFactory.getLogger(DocumentCategoryDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link DocumentCategoryDaoImpl}.
	 */
	private DocumentCategoryDaoImpl() {
	...
	public List<DocumentCategory> get() {
		return (List<DocumentCategory>) applySearch(getSession().createCriteria(_modelType)).list();
	}
	...
	public DocumentCategory getByCode(final String code) {
		...
		validateCode(code);
		return (DocumentCategory) applySearch(getSession().createCriteria(_modelType)
	...
	public DocumentCategory create(final String code, final String description) {
		...
		DocumentCategory object = new DocumentCategory(code, description);
		// Issue save
		save(object);
		return object;
	}
	...
	public DocumentCategory update(final String code, final String description) {
		...
		DocumentCategory object = getByCode(code);
