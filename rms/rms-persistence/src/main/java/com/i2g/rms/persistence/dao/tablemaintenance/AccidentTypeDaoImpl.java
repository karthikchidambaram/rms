 ...
 */
@Repository
public class AccidentTypeDaoImpl extends AbstractHibernateDao<String, AccidentType> implements AccidentTypeDao {

	private final Logger _logger = LoggerFactory.getLogger(AccidentTypeDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link AccidentTypeDaoImpl}.
	 */
	private AccidentTypeDaoImpl() {
	...
	public List<AccidentType> get() {
		return (List<AccidentType>) applySearch(getSession().createCriteria(_modelType)).list();
	}
	...
	public AccidentType getByCode(final String code) {
		...
		validateCode(code);
		return (AccidentType) applySearch(getSession().createCriteria(_modelType)
	...
	public AccidentType create(final String code, final String description) {
		...
		AccidentType object = new AccidentType(code, description);
		// Issue save
		save(object);
		return object;
	}
	...
	public AccidentType update(final String code, final String description) {
		...
		AccidentType object = getByCode(code);
