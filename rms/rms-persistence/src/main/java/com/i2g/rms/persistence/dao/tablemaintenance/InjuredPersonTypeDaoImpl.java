 ...
 */
@Repository
public class InjuredPersonTypeDaoImpl extends AbstractHibernateDao<String, InjuredPersonType> implements InjuredPersonTypeDao {

	private final Logger _logger = LoggerFactory.getLogger(InjuredPersonTypeDaoImpl.class);

	@Autowired
	private HibernateTemplate _hibernateTemplate;

	/**
	 * Creates a new instance of {@link InjuredPersonTypeDaoImpl}.
	 */
	private InjuredPersonTypeDaoImpl() {
		super(InjuredPersonType.class);
	}
	...
	public List<InjuredPersonType> get() {
		return (List<InjuredPersonType>) applySearch(getSession().createCriteria(_modelType)).list();
	}
	...
	public InjuredPersonType getByCode(final String code) {
		...
		validateCode(code);
		return (InjuredPersonType) applySearch(getSession().createCriteria(_modelType)
	...
	public InjuredPersonType create(final String code, final String description) {
		...
		InjuredPersonType object = new InjuredPersonType(code, description);
		// Issue save
		save(object);
		return object;
	}
	...
	public InjuredPersonType update(final String code, final String description) {
		...
		InjuredPersonType object = getByCode(code);
